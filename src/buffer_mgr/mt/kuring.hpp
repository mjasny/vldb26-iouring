...
struct BaseReactor {
    ...
    using WakeDrainFn = void (*)(void*);
    using IdleStopFn = void (*)(void*);
    uint32_t owner_worker_id_ = 0;
    WakeDrainFn wake_drain_hook_ = nullptr;
    void* wake_drain_ctx_ = nullptr;
    IdleStopFn idle_stop_hook_ = nullptr;
    void* idle_stop_ctx_ = nullptr;

    inline void set_owner_worker(uint32_t worker_id) noexcept {
        owner_worker_id_ = worker_id;
    }

    inline void set_wake_drain_hook(void* ctx, WakeDrainFn hook) noexcept {
        wake_drain_ctx_ = ctx;
        wake_drain_hook_ = hook;
    }

    inline void set_idle_stop_hook(void* ctx, IdleStopFn hook) noexcept {
        idle_stop_ctx_ = ctx;
        idle_stop_hook_ = hook;
    }

    inline void maybe_drain_wakeups() noexcept {
        if (wake_drain_hook_) {
            wake_drain_hook_(wake_drain_ctx_);
        }
    }

    inline void maybe_handle_idle_stop(bool& stop) noexcept {
        if (!stop && idle_stop_hook_) {
            idle_stop_hook_(idle_stop_ctx_);
        }
    }
    ...
    inline void spawn(Fiber* f, Fn&& fn) {
        f->owner_worker = owner_worker_id_;
        f->peer = ctx::callcc(
    ...
    inline void park() {
        ...
        if (f->pending_wake.exchange(false, std::memory_order_acq_rel)) {
    ...
    inline bool wake(Fiber* f) noexcept {
        ...
        if (f->state == State::Running) {
            f->pending_wake.store(true, std::memory_order_release);
            return true;
        }
...
struct UringReactor : BaseReactor {
    ...
    void run_loop() {
        while (true) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                fiber_run++;
                // If it yielded, it enqueued itself already; if parked, stays out; if finished, left in arena.
                // drain_cqe();
            }
            if (submitted_io == 0 && pending_io > 0) {
                flush_pending_before_park();
            ...
            if (ready_.empty()) {
                ...
                maybe_handle_idle_stop(local_stop);
            }
            // if (ready_.size() <= 1) {
            //     empty_after_drain++;
            // }
    ...
    void run(bool& stop) {
        while (!stop) {
            ...
            maybe_drain_wakeups();
            if (ready_.empty()) {
                maybe_handle_idle_stop(stop);
            }
        }
    }

    void run(std::atomic<bool>& stop) {
        constexpr uint32_t kStopPollMask = (1u << 16) - 1;
        uint32_t stop_poll_counter = 0;
        while (true) {
            if ((stop_poll_counter++ & kStopPollMask) == 0 &&
    ...
    uint64_t submitted_io = 0;
    uint64_t pending_io = 0;
    ...
    void submit_exact(unsigned expected) {
    ...
    void flush_pending_before_park() {
        if (to_submit == 0) {
            return;
        }
        submit_exact(static_cast<unsigned>(to_submit));
        submitted_io += pending_io;
        pending_io = 0;
        ++num_submits;
        to_submit = 0;
    ...
    int io_batch(int n, Op& op, Prep&& prep) {
        ...
        outstanding_io += op.cqe_left;
        pending_io += op.cqe_left;
        submit_exact(static_cast<unsigned>(n));
        submitted_io += pending_io;
        pending_io = 0;

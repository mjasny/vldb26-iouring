...
#include "../../perfevent_c/perf_event.h"
#include "../arch/arch.h"
...
static int sq_thread_cpu = 70; /* pin above thread to this CPU */
...
static long max_file_size = 0; /* max_file_size */
static int do_perf = 0;
static int setup_mode = 2;
static int do_writes = 0;
...
static void setup_perf(PerfEvent* pevent) {
...
static int prep_more_ios_aio(struct submitter* s, int max_ios, struct iocb* iocbs) {
    ...
    while (index < max_ios) {
        ...
        if (do_writes) {
            io_prep_pwrite(iocb, f->real_fd, s->iovecs[index].iov_base,
...
static void* submitter_sync_fn(void* data) {
    ...
    do {
        ...
        if (do_writes) {
            if (polled)
                ret = pwritev2(f->real_fd, &s->iovecs[0], 1, offset, RWF_HIPRI);
            else
                ret = pwritev2(f->real_fd, &s->iovecs[0], 1, offset, 0);
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt(argc, argv, "d:s:c:b:p:B:F:n:N:O:t:T:a:r:D:R:X:S:P:u:m:e:y:w:h?")) != -1) {
        switch (opt) {
            case 'a':
                aio = !!atoi(optarg);
                break;
